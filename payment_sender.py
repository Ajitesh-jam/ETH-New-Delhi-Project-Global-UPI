...
with open("PaymentSender.json") as f:
    ABI = json.load(f)

def send_native(chain: str, recipient: str, amount: float):
    """Send native coin (ETH, POL, RBTC) on any supported chain"""
    ...
    print("### paying from account:", account.address)
    print("### paying to recipient:", recipient,"\n\n\nAbi: ",ABI)
    contract = w3.eth.contract(address=config["contract"], abi=ABI)
    print("### sending1", amount, chain, "to", recipient,"\n\n\n\nABI: ",ABI)
    amount_wei = w3.to_wei(amount, "ether")
    print("### sending2", amount, chain, "to", recipient)
