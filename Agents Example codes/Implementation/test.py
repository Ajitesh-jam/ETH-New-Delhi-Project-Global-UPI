...
if __name__ == "__main__":
    print("--- Running Agent Tool Function Example ---")
    
    # This call will now handle the entire interactive payment flow.
    # An agent would call this function and wait for the JSON result.
    result_json = convert_and_transfer(
        from_currency="INR",
        to_currency="BTC",
        from_address="user_inr_wallet",
        to_address="user_btc_wallet_address",
        amount=1.50  # 1.50 INR
    )
    ...
    try:
        ...
        if result_data.get("status") == "success":
            print("\nAgent can now proceed, the transaction was successful.")
        else:
            print(f"\nAgent should handle the failure: {result_data.get('message')}")
            
    except json.JSONDecodeError:
        print(f"Received non-JSON response: {result_json}")
